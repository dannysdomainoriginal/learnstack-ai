...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      ></div>
      <aside
        className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:shrink-0 flex flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${
        ...
        <nav className="flex-1 px-3 py-6 space-y-1.5">
          {navLinks.map((link) => (
            <NavLink
              ...
              onClick={toggleSidebar}
              className={({ isActive }) =>
                `group flex items-center gap-3 px-4 py-2.5 text-sm font-semibold rounded-xl transition-all duration-400 ${
                  isActive
